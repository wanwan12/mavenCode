package com.wanwan.mavencode.widget;
...
public class CustomProgressBar extends View {
    ...
    private boolean showCircularPoint;
    private int circularPointRadius;
    private int circularPointColor;
    ...
    private int DEFAULT_BG_COLOR = Color.parseColor("#FFFFFF");
    ...
    public CustomProgressBar(Context context, AttributeSet attrs,
        ...
        try {
            mMax = a.getInt(R.styleable.CustomProgressBar_maxProgress, DEFAULT_MAX);
            mProgress = a.getInt(R.styleable.CustomProgressBar_progress, DEFAULT_PROGRESS);
            mRadius = (int) a.getDimension(R.styleable.CustomProgressBar_progressRadius, DEFAULT_RADIUS);
            mBgColor = a.getColor(R.styleable.CustomProgressBar_bgColor, DEFAULT_BG_COLOR);
            mBoardColor = a.getColor(R.styleable.CustomProgressBar_progressColor, DEFAULT_BORDER_COLOR);
            mProgressColor = a.getColor(R.styleable.CustomProgressBar_progressColor, DEFAULT_PROGRESS_COLOR);
            mProgressDescColor = a.getColor(R.styleable.CustomProgressBar_progressDescColor, DEFAULT_PROGRESS_DESC_COLOR);
            mBorderWidth = (int) a.getDimension(R.styleable.CustomProgressBar_borderWidth, DEFAULT_BORDER_WIDTH);
            mProgressDesc = a.getString(R.styleable.CustomProgressBar_progressDesc);
            mIsShowDesc = a.getBoolean(R.styleable.CustomProgressBar_isShowDesc, DEFAULT_ISSHOWDESC);
            showCircularPoint = a.getBoolean(R.styleable.CustomProgressBar_showCircularPoint, DEFAULT_ISSHOWDESC);
            circularPointRadius = (int) a.getDimension(R.styleable.CustomProgressBar_circularPointRadius, DEFAULT_RADIUS);
            progressHeight = (int) a.getDimension(R.styleable.CustomProgressBar_progressHeight, DEFAULT_RADIUS);
            circularPointColor =  a.getColor(R.styleable.CustomProgressBar_circularPointColor, DEFAULT_PROGRESS_DESC_COLOR);
    ...
    private void drawBorder(Canvas canvas) {
        ...
        int top = (mHeight - progressHeight) / 2 + mBorderWidth / 2;
        int right = mWidth - mBorderWidth / 2;
        int bottom = mHeight - mBorderWidth / 2 - (mHeight - progressHeight) / 2;
    ...
    private void drawBg(Canvas canvas) {
        int radius = mRadius - mBorderWidth * 2;

        int left = mBorderWidth;
        int top = (mHeight - progressHeight) / 2 + mBorderWidth;
        int right = mWidth - mBorderWidth;
        int bottom = mHeight - mBorderWidth - (mHeight - progressHeight) / 2;
        ...
        path.arcTo(new RectF(right - 2 * radius, bottom - 2 * radius, right,
                bottom), 0, 90);
        path.lineTo(left + radius, bottom);
        path.arcTo(new RectF(left, bottom - 2 * radius, left + 2 * radius,
                bottom), 90, 90);
        path.lineTo(left, top + radius);
        path.arcTo(new RectF(left, top, left + 2 * radius, top + 2 * radius),
                180, 90);
        path.close();
    ...
    private void drawProgress(Canvas canvas) {
        ...
        float top = (mHeight - progressHeight) / 2 + mBorderWidth * .5f;
        float right = mWidth - mBorderWidth * .5f;
        float bottom = mHeight - mBorderWidth * .5f - (mHeight - progressHeight) / 2;
    ...
    private void drawCirclePoint(Canvas canvas) {
        ...
        int left = (int)((mProgress * 1.f / mMax)*(mWidth-circularPointRadius*2));
        int top = (mHeight - circularPointRadius*2) / 2;
        int right =(int)((mProgress * 1.f / mMax)*(mWidth-circularPointRadius*2)) + 2*circularPointRadius;
        int bottom = (mHeight - circularPointRadius*2) / 2 +circularPointRadius*2 ;
        ...
        mPaint.setStrokeJoin(Paint.Join.MITER);
        mPaint.setAntiAlias(true);
        mPaint.setColor(circularPointColor);
